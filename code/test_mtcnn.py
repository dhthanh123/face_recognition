...
class capture_video(QThread):
    ...
    def ca_disk(self, img_cropped, embedding_list):
        min_dist = min_dist_idx  = 2
        try:
            emb = resnet.run(None, {"actual_input_1": img_cropped})
                ...
                if results is not None:
                    for idx, result in enumerate(results):
                        if result['confidence'] > 0.90:
                            ...
                            x2, y2 = x1 + 160, y1 + 160
                            picture = cv_img[y1:y2, x1:x2]
                            picture = np.rollaxis(picture, 2, 0)
                            picture = picture.astype(np.float32)
                            picture = np.expand_dims(picture, axis=0)

                            min_dist_idx, min_dist = self.ca_disk(picture, embedding_list)
                            staff_infor = name_list[min_dist_idx]
                            print(min_dist)
                            if min_dist < 0.7:
                                staff_code, staff_name = staff_infor.split('_')
                                cv2.rectangle(cv_img, (x1, y1), (x1 + width, y1 + height), (0, 155, 255), 2)
                                ...
                                cv_img = cv2.putText(cv_img, 'Press Enter to complete.....', (20, 400),
                                                     cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
                        end_time = time.time()
                        fps = 1 / (end_time - start_time)
                        fps = str(fps)
                        cv_img = cv2.putText(cv_img, 'FPS: ' + fps[0:2], (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1,(0, 255, 0), 2, cv2.LINE_AA)
                            #cv_img = img_with_dets[y1:y2, x1:x2]
                            #print(cv_img.shape)

                            # cv2.circle(cv_img, (keypoints['left_eye']), 2, (0, 155, 255), 2)
                            # cv2.circle(cv_img, (keypoints['right_eye']), 2, (0, 155, 255), 2)
                            # cv2.circle(cv_img, (keypoints['nose']), 2, (0, 155, 255), 2)
                            # cv2.circle(cv_img, (keypoints['mouth_left']), 2, (0, 155, 255), 2)
                            # cv2.circle(cv_img, (keypoints['mouth_right']), 2, (0, 155, 255), 2)
                            # box = result['box']
